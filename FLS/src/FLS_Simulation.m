...
dr_m = 10 * c0 / (2 * BW);  % 10倍距离分辨率 ≈ 75 mm (组2距离向间距，约5个深度像素)
...
pos_target = [
    ...
    % 组2 @ z=10m: 十字形5目标（中心+方位向±+距离向±）
    0,                     0, 10;         % 目标3 十字中心
    -10*tand(delta_deg/2), 0, 10;         % 目标4 十字左（方位向）
    +10*tand(delta_deg/2), 0, 10;         % 目标5 十字右（方位向）
    0,                     0, 10-dr_m/2; % 目标6 十字上（距离向）
    0,                     0, 10+dr_m/2; % 目标7 十字下（距离向）

    % 孤立参考目标 @ z=8m, 方位角3° (减小坐标系误差)
    8*tand(3),             0,  8;         % 目标8
];

amp_target = [1; 1; 1; 1; 1; 1; 1; 2];  % T8(孤立目标) 幅度×2 补偿az=3°处增益衰减
...
depth_axis   = linspace(5.0, 11.0, 384)';            % 深度轴: 5-11m, 384点
azimuth_axis = linspace(-2, 7, 512) / 180 * pi;    % 方位轴: -2°~7°, 512点
...
% --- 方位向 PSF: 从孤立目标 T8 (z=8m, az=3°) 实测 ---
az_step_rad = (azimuth_axis(end) - azimuth_axis(1)) / (N_a - 1);

% 找孤立目标 T8 在图像中的像素位置 (z=8m, az=3°，与待分辨目标无重叠)
[~, idx_r_t5] = min(abs(depth_axis - 8.0));
[~, idx_a_t5] = min(abs(az_deg - 3.0));
...
tgt_az = [-delta_deg/2,  delta_deg/2, ...             % 组1 @ z=6m (T1,T2)
          0, -delta_deg/2, delta_deg/2, 0, 0, ...     % 组2十字 @ z=10m (T3-T7)
          3];                                           % 孤立目标T8 @ az=3°
tgt_r  = [6, 6, 10, 10, 10, 10-dr_m/2, 10+dr_m/2, 8];
