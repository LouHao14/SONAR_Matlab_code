...
%% [新增] 旁瓣验证目标
% 目的：验证 FISTA 对强目标旁瓣的抑制能力
% 设计：T9(0 dB, az=+5°) 产生旁瓣覆盖 az≈0° 区域
%       T10(-20 dB, az=0°, z=8.3m) 为旁瓣区弱目标，测试是否被遮蔽
pos_target = [pos_target;
    +8*tand(5.0), 0, 8.0;   % T9:  强目标, az=+5°,  幅度 1.0 ( 0 dB)
    0,            0, 8.3;    % T10: 弱目标, az= 0°,  幅度 0.1 (-20 dB)
];
amp_target = [amp_target; 1.0; 0.1];

fprintf('目标定义: bw=%.3f°, delta=%.3f° (%.0f%% BW), 10个目标\n', ...
...
title(sprintf('10目标场景: delta=%.3f° (bw=%.3f°)', delta_deg, bw_deg));
xlim([-2, 8]); ylim([5, 11]);
text(tgt_az_preview+0.1, tgt_r_preview, ...
    {'T1','T2','T3','T4','T5','T6','T7','T8','T9','T10'}, 'FontSize', 10, 'Color', 'k');
drawnow;
...
tgt_az = [-delta_deg/2,  delta_deg/2, ...             % 组1 @ z=6m (T1,T2)
          ...
          0, ...                                        % T8: 孤立目标 @ az=0°
          5.0, 0];                                     % T9(强) @ az=5°, T10(弱) @ az=0°
tgt_r  = [6, 6, 10, 10, 10-dr_m/2, 10+dr_m/2, 10, 8, 8.0, 8.3];
...
%% [新增] 旁瓣抑制验证：局部放大图 + 定量输出

% --- 计算 T9 在 az≈0° 处最近旁瓣的理论角度位置 ---
az_T9_rad           = 5.0 * pi/180;
delta_sin_arr       = lambda / (probe.N * probe.pitch);  % 旁瓣间距（sin 空间）
n_near              = round(sin(az_T9_rad) / delta_sin_arr);
az_sidelobe_theory_deg = asind(sin(az_T9_rad) - n_near * delta_sin_arr);

% --- 在图像中找 T10 弱目标的像素坐标 ---
[~, idx_r_t10] = min(abs(depth_axis - 8.3));
[~, idx_a_t10] = min(abs(az_deg  -  0.0));
[~, idx_r_t9 ] = min(abs(depth_axis - 8.0));
[~, idx_a_sl  ] = min(abs(az_deg  - az_sidelobe_theory_deg));

% 读取 dB 值
cbf_at_t10   = cbf_db_vis(idx_r_t10, idx_a_t10);
fista_at_t10 = fista_db(idx_r_t10,   idx_a_t10);
cbf_at_sl    = cbf_db_vis(idx_r_t9,  idx_a_sl);
fista_at_sl  = fista_db(idx_r_t9,    idx_a_sl);

% --- 旁瓣验证局部放大图 ---
az_sl_lim  = [-2, 1];     % 方位角局部范围 [°]（聚焦干扰区）
dep_sl_lim = [7.5, 9.0];  % 深度局部范围 [m]

fig_sl = figure('Name', '旁瓣抑制验证 (局部放大)', ...
                'Position', [100, 100, 1200, 550], 'Color', 'w');

ax_sl1 = subplot(1, 2, 1);
imagesc(az_deg, depth_axis, cbf_db_vis);
colormap(ax_sl1, hot); clim([-60 0]); colorbar;
hold on;
plot(0.0, 8.3, 'c+', 'MarkerSize', 12, 'LineWidth', 1.8);  % T10 弱目标
text(0.05, 8.3, 'T10(弱,-20dB)', 'Color', 'c', 'FontSize', 9);
xline(az_sidelobe_theory_deg, 'w--', ...
    sprintf('T9旁瓣落点 ≈%.2f°', az_sidelobe_theory_deg), ...
    'LineWidth', 1.2, 'FontSize', 9, 'LabelVerticalAlignment', 'bottom');
hold off;
title('CBF / DAS — 强目标旁瓣是否遮蔽弱目标', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('方位角 [°]'); ylabel('深度 [m]');
set(ax_sl1, 'XLim', az_sl_lim, 'YLim', dep_sl_lim);

ax_sl2 = subplot(1, 2, 2);
imagesc(az_deg, depth_axis, fista_db);
colormap(ax_sl2, hot); clim([-60 0]); colorbar;
hold on;
plot(0.0, 8.3, 'c+', 'MarkerSize', 12, 'LineWidth', 1.8);
text(0.05, 8.3, 'T10(弱,-20dB)', 'Color', 'c', 'FontSize', 9);
xline(az_sidelobe_theory_deg, 'w--', ...
    sprintf('T9旁瓣落点 ≈%.2f°', az_sidelobe_theory_deg), ...
    'LineWidth', 1.2, 'FontSize', 9, 'LabelVerticalAlignment', 'bottom');
hold off;
title('CBF + FISTA — 旁瓣被压制，弱目标得以恢复', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('方位角 [°]'); ylabel('深度 [m]');
set(ax_sl2, 'XLim', az_sl_lim, 'YLim', dep_sl_lim);

linkaxes([ax_sl1, ax_sl2], 'xy');
sgtitle('旁瓣抑制性能验证 — T9强目标旁瓣 vs T10弱目标可见性', ...
    'FontSize', 13, 'FontWeight', 'bold');
drawnow;

% --- 定量输出 ---
fprintf('\n=== 旁瓣抑制验证 ===\n');
fprintf('弱目标真实位置 (az, r):            ( 0°, 8.3m)\n');
fprintf('CBF  在弱目标位置的幅度 [dB]:      %5.1f dB\n', cbf_at_t10);
fprintf('FISTA在弱目标位置的幅度 [dB]:      %5.1f dB\n', fista_at_t10);
fprintf('强目标第一旁瓣理论位置 [deg]:      %5.2f°\n',   az_sidelobe_theory_deg);
fprintf('CBF  在旁瓣落点的幅度 [dB]:        %5.1f dB  (越高越差)\n', cbf_at_sl);
fprintf('FISTA在旁瓣落点的幅度 [dB]:        %5.1f dB  (越低越好)\n', fista_at_sl);
