%%
% =========================================================================
% 文件名:    FLS_CBF_MVDR_V1.m
% 版本:      1.0
% 描述:      前视声纳 (FLS) 仿真与波束形成算法对比验证平台
%            基于 Field II 生成回波数据，利用 USTB 进行 CBF 与 MVDR 成像对比
% 作者:      [楼浩/OIT]
% 创建日期:   2026-01-19
% 最后修改:   2026-01-19
% -------------------------------------------------------------------------
% 依赖工具箱:
%   1. Field II Simulation Program (http://field-ii.dk)
%   2. USTB (UltraSound ToolBox)   (http://www.ustb.no)
% -------------------------------------------------------------------------
% 功能模块:
%   1. 仿真场景生成: 3D 参数化海底地形 + STL 目标模型 (球体)
%   2. 声学物理模拟: 128阵元线阵, Chirp 信号发射, 物理声场计算 (Field II)
%   3. 信号处理管道: 
%      - CBF (Conventional Beamforming / Delay-and-Sum)
%      - MVDR (Minimum Variance Distortionless Response / Capon)
%   4. 可视化: 
%      - 3D 场景与探测视锥 (FOV) 可视化
%      - B-mode 声纳图像对比 (DAS vs MVDR)
% =========================================================================
...
% 目标 1 (原来的水雷)
t1_pos = [2, 10, 8]; % [x, y, z] 注意这里是 Field II 坐标

% 目标 2 (干扰目标，距离非常近)
% 让我们把他在横向(X)上只移动 0.6 米 (对于 8米远的距离，这大约是 4度角)
% CBF 的物理波束宽度通常在 5-10度，所以 CBF 绝对分不开这两个点。
t2_pos = [1.4, 10, 8]; 

% 合并目标
pos_target = [t1_pos; t2_pos];
amp_target = [100; 100]; % 两个强度一样的目标

% 坐标转换 (适配你的代码逻辑)
pos_target = pos_target(:, [1 3 2]); % 重排为 [x, z, y] 用于 Field II
...
%% ========================================================================
%  Richardson-Lucy (RL) 去卷积 - 最终修正版 (修复句柄污染bug)
% ========================================================================

% 1. 准备数据
% 必须从原始 CBF 数据中提取，确保此时它还没被污染
img_orig = abs(b_data_cbf.data); 

% 还原形状
img_shape = [sca.N_depth_axis, sca.N_azimuth_axis];
img_orig = reshape(img_orig, img_shape);

% 归一化
img_orig = img_orig / max(img_orig(:)); 

% 2. 执行 RL 算法
psf_size = 15; 
sigma_z = 2; 
sigma_x = 4; 
PSF = fspecial('gaussian', psf_size, sigma_x); 
iterations = 15; 
img_rl = deconvlucy(img_orig, PSF, iterations);

% -------------------------------------------------------------------------
% [核心修复] : 创建一个全新的独立对象，而不是复制句柄
% -------------------------------------------------------------------------
% 1. 新建一个空的 beamformed_data 对象
b_data_rl = uff.beamformed_data(); 

% 2. 只把必要的 scan 信息给它 (让它知道怎么画扇形)
b_data_rl.scan = b_data_cbf.scan; 

% 3. 填入 RL 计算出的新数据
b_data_rl.data = img_rl(:); 
% 此时 b_data_rl 和 b_data_cbf 互不干扰！

% -------------------------------------------------------------------------
% 4. 对比显示
% -------------------------------------------------------------------------
figure('Name', '真·CBF vs RL (修复变量污染)', 'Position', [100, 100, 1200, 500], 'Color', 'w');

% --- 左图：原始 CBF (现在应该是真的原始模糊图像了) ---
subplot(1, 2, 1);
b_data_cbf.plot(subplot(1,2,1)); 
title('原始 CBF/DAS (应比较模糊)'); % 标题提醒
colormap(hot); 
clim([-60 0]); 
set(gca, 'XDir', 'normal'); 
xlabel('横向 X [m]'); ylabel('距离 Z [m]');
...
% --- 右图：RL 去卷积 ---
subplot(1, 2, 2);
b_data_rl.plot(subplot(1,2,1)); % 调用新对象的绘图
title(['RL 超分辨率 (应明显变细) - 迭代 ' num2str(iterations)]);
colormap(hot); 
clim([-60 0]); 
set(gca, 'XDir', 'normal'); 
xlabel('横向 X [m]'); ylabel('距离 Z [m]');

disp('对比完成！请确认左图现在是否变回了模糊的状态。');